    ...
    return "\n".join(
        [
            ...
            *market_context,
            ...
            "Lead with a decisive call in consensus.title, consensus.summary, and every agent thesis: write 매수 우위, 매도 우위, or 관망 first, then give the exact chart condition that supports it.",
            "Never output the English labels buy-side, sell-side, BUY, SELL, or WAIT in Korean fields. Use 매수 우위, 매도 우위, or 관망 consistently.",
            "Do not dilute a supported call with stacked hedges such as 'may, might, possibly' repeated in one opinion. If evidence is insufficient, say 관망 plainly and name the single next confirmation trigger.",
            "Aggressive wording must remain evidence-bounded: never turn confidence into certainty, invent a trade entry, or hide the invalidation condition.",
