    ...
    return "\n".join(
        [
            ...
            *validation_directives,
            ...
            "Set trade_plan.risk_reward to a numeric risk:reward string with ASCII colon, e.g. 1:2.0. Never use 2R, a bare decimal, percentage, or translated separator; numeric ratios are language-independent.",
            "Before returning, verify the arithmetic: bearish (entry-target)/(stop-entry) >= 1.8; bullish (target-entry)/(entry-stop) >= 1.8. Prefer single numeric prices; if using ranges, their first numeric prices must satisfy the same calculation. Do not move the stop inside visible invalidation or invent a target merely to reach the ratio.",
