...
def _session_prompt(request: FlirtistProductSessionRequest, fallback: FlirtistProductSessionResponse) -> str:
    ...
    return "\n".join(
        [
            ...
            "For screenshot images, first silently build an ordered transcript from visible chat bubbles: top-to-bottom order, wrapped lines grouped into the same bubble, left-side bubbles = Them, right-side bubbles = Me.",
            ...
            *CHAT_ONLY_VISUAL_TEXT_CONTRACT,
            "chatPreview must contain only Me/Them chat messages from visible bubbles; exclude every non-chat visual fragment even if it appears in Request JSON.",
            ...
            *STYLE_PURPOSE_CONTRACT,
            ...
            "Every reply must be copy-ready text the user can send. Never start with speaker labels, visual fragments, Message..., Them:, Me:, or explanations.",
