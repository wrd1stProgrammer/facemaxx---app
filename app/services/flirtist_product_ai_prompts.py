...
CHAT_ONLY_VISUAL_TEXT_CONTRACT: Final[tuple[str, ...]] = (
...
def _session_prompt(request: FlirtistProductSessionRequest, fallback: FlirtistProductSessionResponse) -> str:
    ...
    return "\n".join(
        [
            ...
            "If a Cloudinary screenshot URL is attached, inspect the image itself as the primary source. Do not rely on client-provided transcript text from screenshot preprocessing.",
            "For reply_coach screenshot images, first classify contentKind exactly: chat for message threads, bio for dating profile/bio/profile-card screenshots.",
            "Return contentKind in the JSON. If uncertain, choose chat only when there are visible message bubbles from both sides; choose bio when the screenshot is a profile, prompt, interests list, or dating bio with no existing conversation.",
            "For contentKind=chat, silently build an ordered transcript from visible chat bubbles: top-to-bottom order, wrapped lines grouped into the same bubble, left-side bubbles = Them, right-side bubbles = Me.",
            "For contentKind=bio, extract only profile/bio details, prompts, interests, captions, and visible profile context, and do not pretend there is an existing chat.",
            "For contentKind=bio, generate first-message openers that Me can send to the profile owner. Each opener must mention or naturally use a real profile/bio detail and give Them an easy way to answer.",
            "For long stitched screenshots, scan the full image height and preserve chronological order from top to bottom.",
            "If manual transcript text is provided without an image, infer the latest actionable exchange from the whole transcript, not only the final short reaction.",
            ...
            f"For contentKind=chat, answer the latest meaningful Them message: {latest_them_message or '<infer latest Them message from transcript>'}. If the final line is Me, suggest a low-pressure follow-up from Me instead of inventing Them's reply.",
            ...
            "Ignore app chrome, input placeholders, timestamps, icon labels, and visual UI noise such as Message..., Type a message, Send, AI 추천 답장, Get NSFW Reply, FLIRTIST, or 집중할 키워드.",
            *CHAT_ONLY_VISUAL_TEXT_CONTRACT,
            "For contentKind=chat, chatPreview must contain only Me/Them chat messages from visible bubbles; exclude every non-chat visual fragment even if it appears in Request JSON.",
            "For contentKind=bio, chatPreview should contain concise profile evidence rows using role=them or role=system; never create fake Me/Them chat messages.",
            ...
            *STYLE_PURPOSE_CONTRACT,
            ...
            "Every reply or opener must be copy-ready text the user can send. Never start with speaker labels, visual fragments, Message..., Them:, Me:, or explanations.",
            "For contentKind=chat, ground every reply in the last meaningful chat message. If there is enough context, avoid generic prompts like 'tell me more' unless phrased around a specific detail.",
            "For contentKind=bio, ground every opener in profile/bio details. Avoid generic openers like 'hey', 'how are you', 'tell me more', or broad compliments that could fit any profile.",
            "Do not quote a full incoming message or full bio line inside the reply. A short callback is fine; parroting the screenshot text is a failure.",
            ...
            "Quality bar for every reply/opener: 1) references the exact situation or profile without quoting the whole message, 2) sounds like a real text, 3) gives the other person an easy next response, 4) does not suddenly escalate intimacy, 5) is specific enough that it would be wrong for a different chat/profile.",
...
def _style_prompt(request: FlirtistReplyStyleRequest, fallback: FlirtistReplyStyleResponse) -> str:
    ...
    return "\n".join(
        [
            ...
            f"Target language: {language_name(target_language)}. Locale: {request.locale}.",
            f"contentKind: {request.contentKind}.",
            f"Cultural tone guide: {culture_guidance(target_language)}",
            "All returned reply text, labels, summary, nextMove, and whyItWorks must be in the target language.",
            "If contentKind=bio, generate more first-message openers from profile/bio details. Use profile/bio details as the evidence, and do not pretend there is an existing chat.",
            ...
            *STYLE_PURPOSE_CONTRACT,
            ...
            "whyItWorks must explain why that purpose fits this chat, not just restate that the reply is good.",
            "Ignore text placeholders and UI chrome such as Message..., Type a message, Send, AI 추천 답장, Get NSFW Reply, FLIRTIST, or 집중할 키워드.",
            *CHAT_ONLY_VISUAL_TEXT_CONTRACT,
            ...
            "For contentKind=bio, those alternatives are more first-message openers, not replies to an incoming message.",
